...
ordersList=new OrdersList(*other.ordersList); //deep copy
}
...
player has no ownership*/
 Player::~Player(){
 delete ordersList; //delete the Orderlist
...
//add an order to the player's order list. Player owns the order list and it will delete it in the destructor
    ...
    if(ord!=nullptr){
     ...
     if (ordersList) {
      ordersList->getList().push_back(ord);
     }
      }
}
...
bool Player::equals(Player* player2){
 if (this->getName() != player2->getName())
  return false;
